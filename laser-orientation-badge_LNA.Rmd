...
-   *What are these data, where is it from? What kinds of students? *

-   *Are rows unique students?*

-   *What do the headers mean (int, pc, uv)?* 
...
-   *Rows represent observations, they are each of the same "unit", whether it is a unique student, or responses (by student id). Whatever the unit, there are the same variables/metrics (columns given for each row).*

What columns typically (or you think may) represent:

-   *Columns are the variables that describe or of interest to investigate. So if rows are students, columns can be id or grouping variables, like student id, demographic variables, course info, student responses or metrics.* 
...
-   *The number of rows decreases from 438 to 203 when changing the cutoff from 70 to 90.*
...
```{r}
d |> 
  select(student_id, FinalGradeCEMS, Gender, TimeSpent) |>
...
```{r}
ggplot(d, aes(x = TimeSpent, y = FinalGradeCEMS)) +
